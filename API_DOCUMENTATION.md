...
| **DELETE** | `/questions/{id}/answer` | Delete the authenticated user's answer to a question. Removes the row from `user_answers` and the tracking row from `question_user` (which stores the anonymous hash), allowing the user to answer the same question again later. The deletion is performed atomically inside a transaction. Only answers to *approved* questions can be deleted. | Bearer access token | – | *200 OK* – `{ "message": "Answer deleted" }`. *404* if the user has not answered this question or the question does not exist. Errors: 401 (unauthenticated), 500 (DB error). |
| **GET** | `/questions/{id}/results` | Get aggregated results for a question. | Bearer access token | – | *200 OK* – tally per answer. |
| **POST** | `/questions/restSearch` | Search/filter questions via a JSON body. Filters: `language` (exact match), `search` (case‑insensitive substring on the question text), `categoryIds` (match any of the given category ids), `age` (question `min_age` >= value). Supports pagination via `offset` (default 0) and `limit` (default 50, max 1000). Optional `unanswered_only` (boolean, default `false`) — when `true`, filters out questions the authenticated user has already answered (only applies to authenticated endpoints). | No | ```json
{ "language": "string", "search": "string", "categoryIds": [1, 2, 3], "age": 0, "offset": 0, "limit": 50, "unanswered_only": false }
...
| **POST** | `/questions/restSearch-with-auth` | Search/filter questions via a JSON body (same filters as `/questions/restSearch`), but requires authentication. Returns an additional `answered` field per question indicating whether the authenticated user has already answered it. When `unanswered_only` is `true`, only questions the user has **not** answered are returned (client-side pagination is applied for this filter). | Bearer access token | ```json
{ "language": "string", "search": "string", "categoryIds": [1, 2, 3], "age": 0, "offset": 0, "limit": 50, "unanswered_only": false }
``` | *200 OK* – array of question objects with `answered` flag: `[ { "id": 123, "text": "...", "language": "en", "category_id": 1, "category_name": "Politics", "special_category": "none", "answered": false }, ... ]`. Errors: 400 (invalid JSON body), 401 (unauthenticated), 500 (DB error). |
...
| **GET** | `/countries` | Return all entries from the **countries** reference table. Unrestricted endpoint; returns each country's ISO 3166‑1 alpha‑2 `code` and human‑readable `name`. Useful for client‑side UI dropdowns. | No | – | *200 OK* – array of country objects (`{ "code": "string", "name": "string" }`). |
| **GET** | `/regions` | Return all entries from the **regions** reference table. Unrestricted endpoint; returns each ISO 3166‑2 subdivision `code`, `name`, and the parent `country_code`. Enables filtered region selection and UI validation of region inputs. | No | – | *200 OK* – array of region objects (`{ "code": "string", "name": "string", "country_code": "string" }`). |
