...
void RestfulAnswerOptionsCtrlBase::getOne(
...
void RestfulAnswerOptionsCtrlBase::updateOne(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback,
    AnswerOptions::PrimaryKeyType&& id) {
  ...
  try {
    if (isMasquerading()) {
      if (!AnswerOptions::validateMasqueradedJsonForUpdate(
...
void RestfulAnswerOptionsCtrlBase::deleteOne(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback,
    AnswerOptions::PrimaryKeyType&& id) {
  auto dbClientPtr = getDbClient();
...
void RestfulAnswerOptionsCtrlBase::get(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback) {
  auto dbClientPtr = getDbClient();
  drogon::orm::Mapper<AnswerOptions> mapper(dbClientPtr);
...
void RestfulAnswerOptionsCtrlBase::create(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback) {
  auto jsonPtr = req->jsonObject();
  if (!jsonPtr) {
  ...
  if (isMasquerading()) {
    if (!AnswerOptions::validateMasqueradedJsonForCreation(
            *jsonPtr, masqueradingVector(), err)) {
      Json::Value ret;
      ret["error"] = err;
      auto resp = HttpResponse::newHttpJsonResponse(ret);
  ...
  try {
    AnswerOptions object =
        (isMasquerading() ? AnswerOptions(*jsonPtr, masqueradingVector())
...
RestfulAnswerOptionsCtrlBase::RestfulAnswerOptionsCtrlBase()
    : RestfulController({"id", "question_id", "text"}) {
  /**
