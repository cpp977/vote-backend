...
void RestfulCategoriesCtrlBase::getOne(
...
void RestfulCategoriesCtrlBase::updateOne(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback,
    Categories::PrimaryKeyType&& id) {
  ...
  try {
    if (isMasquerading()) {
      if (!Categories::validateMasqueradedJsonForUpdate(
...
void RestfulCategoriesCtrlBase::deleteOne(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback,
    Categories::PrimaryKeyType&& id) {
  auto dbClientPtr = getDbClient();
...
void RestfulCategoriesCtrlBase::get(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback) {
  auto dbClientPtr = getDbClient();
  drogon::orm::Mapper<Categories> mapper(dbClientPtr);
...
void RestfulCategoriesCtrlBase::create(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback) {
  auto jsonPtr = req->jsonObject();
  if (!jsonPtr) {
  ...
  if (isMasquerading()) {
    if (!Categories::validateMasqueradedJsonForCreation(
            *jsonPtr, masqueradingVector(), err)) {
      Json::Value ret;
      ret["error"] = err;
      auto resp = HttpResponse::newHttpJsonResponse(ret);
  ...
  try {
    Categories object =
        (isMasquerading() ? Categories(*jsonPtr, masqueradingVector())
                          : Categories(*jsonPtr));
    auto dbClientPtr = getDbClient();
    auto callbackPtr =
        std::make_shared<std::function<void(const HttpResponsePtr&)>>(
