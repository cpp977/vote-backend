...
void RestfulQuestionsCtrlBase::getOne(
...
void RestfulQuestionsCtrlBase::updateOne(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback,
    Questions::PrimaryKeyType&& id) {
  ...
  try {
    if (isMasquerading()) {
      if (!Questions::validateMasqueradedJsonForUpdate(
...
void RestfulQuestionsCtrlBase::deleteOne(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback,
    Questions::PrimaryKeyType&& id) {
  auto dbClientPtr = getDbClient();
...
void RestfulQuestionsCtrlBase::get(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback) {
  auto dbClientPtr = getDbClient();
  drogon::orm::Mapper<Questions> mapper(dbClientPtr);
...
void RestfulQuestionsCtrlBase::create(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback) {
  auto jsonPtr = req->jsonObject();
  if (!jsonPtr) {
  ...
  if (isMasquerading()) {
    if (!Questions::validateMasqueradedJsonForCreation(
            *jsonPtr, masqueradingVector(), err)) {
      Json::Value ret;
      ret["error"] = err;
      auto resp = HttpResponse::newHttpJsonResponse(ret);
  ...
  try {
    Questions object =
        (isMasquerading() ? Questions(*jsonPtr, masqueradingVector())
                          : Questions(*jsonPtr));
    auto dbClientPtr = getDbClient();
    auto callbackPtr =
        std::make_shared<std::function<void(const HttpResponsePtr&)>>(
