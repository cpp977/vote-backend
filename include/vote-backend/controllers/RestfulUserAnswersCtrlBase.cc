...
void RestfulUserAnswersCtrlBase::getOne(
...
void RestfulUserAnswersCtrlBase::updateOne(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback,
    UserAnswers::PrimaryKeyType&& id) {
  ...
  try {
    if (isMasquerading()) {
      if (!UserAnswers::validateMasqueradedJsonForUpdate(
...
void RestfulUserAnswersCtrlBase::deleteOne(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback,
    UserAnswers::PrimaryKeyType&& id) {
  auto dbClientPtr = getDbClient();
...
void RestfulUserAnswersCtrlBase::get(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback) {
  auto dbClientPtr = getDbClient();
  drogon::orm::Mapper<UserAnswers> mapper(dbClientPtr);
...
void RestfulUserAnswersCtrlBase::create(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback) {
  auto jsonPtr = req->jsonObject();
  if (!jsonPtr) {
  ...
  if (isMasquerading()) {
    if (!UserAnswers::validateMasqueradedJsonForCreation(
            *jsonPtr, masqueradingVector(), err)) {
      Json::Value ret;
      ret["error"] = err;
      auto resp = HttpResponse::newHttpJsonResponse(ret);
  ...
  try {
    UserAnswers object =
        (isMasquerading() ? UserAnswers(*jsonPtr, masqueradingVector())
                          : UserAnswers(*jsonPtr));
    auto dbClientPtr = getDbClient();
    auto callbackPtr =
        std::make_shared<std::function<void(const HttpResponsePtr&)>>(
    ...
    mapper.insert(
        ...
        [req, callbackPtr, this](UserAnswers newObject) {
          (*callbackPtr)(
              HttpResponse::newHttpJsonResponse(makeJson(req, newObject)));
        },
        [callbackPtr](const DrogonDbException& e) {
          LOG_ERROR << e.base().what();
          Json::Value ret;
...
RestfulUserAnswersCtrlBase::RestfulUserAnswersCtrlBase()
    : RestfulController(
          {"id", "question_id", "answer_id", "tags", "created_at"}) {
  /**
