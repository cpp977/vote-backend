...
const std::string& Users::getValueOfPasswordHash() const noexcept {
  static const std::string defaultValue = std::string();
  if (passwordHash_) return *passwordHash_;
  return defaultValue;
}
...
const ::trantor::Date& Users::getValueOfUpdatedAt() const noexcept {
  static const ::trantor::Date defaultValue = ::trantor::Date();
  if (updatedAt_) return *updatedAt_;
  return defaultValue;
}
...
std::string Users::toString() const { return toJson().toStyledString(); }
...
bool Users::validateJsonForCreation(const Json::Value& pJson,
  ...
  if (pJson.isMember("email")) {
    if (!validJsonOfField(2, "email", pJson["email"], err, true)) return false;
  } else {
...
bool Users::validateJsonForUpdate(const Json::Value& pJson, std::string& err) {
  ...
  if (pJson.isMember("email")) {
    if (!validJsonOfField(2, "email", pJson["email"], err, false)) return false;
  }
  if (pJson.isMember("password_hash")) {
    if (!validJsonOfField(3, "password_hash", pJson["password_hash"], err,
                          false))
