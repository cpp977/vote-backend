...
public:
    JwtServiceFixture() : service("test-secret-key-for-unit-tests-only-32chars", 15, 7) {}
...
TEST_CASE_FIXTURE(JwtServiceFixture, "Constructor initializes with correct values") {
...
TEST_CASE_FIXTURE(JwtServiceFixture, "Generate access token returns non-empty string") {
...
TEST_CASE_FIXTURE(JwtServiceFixture, "Generate access token contains correct claims") {
    int64_t user_id = 42;
...
TEST_CASE_FIXTURE(JwtServiceFixture, "Generate refresh token returns non-empty string") {
...
TEST_CASE_FIXTURE(JwtServiceFixture, "Generate refresh token contains correct claims") {
    int64_t user_id = 789;
...
TEST_CASE_FIXTURE(JwtServiceFixture, "Verify token with wrong signature returns null") {
...
TEST_CASE_FIXTURE(JwtServiceFixture, "Verify token with wrong issuer returns null") {
    ...
    auto token = jwt::create()
        ...
        .sign(jwt::algorithm::hs256{"test-secret-key-for-unit-tests-only-32chars"});
...
TEST_CASE_FIXTURE(JwtServiceFixture, "Extract JTI from invalid token returns empty") {
...
TEST_CASE_FIXTURE(JwtServiceFixture, "Different user IDs produce different tokens") {
...
TEST_CASE_FIXTURE(JwtServiceFixture, "Same user ID produces different tokens (due to timestamps)") {
...
TEST_CASE_FIXTURE(JwtServiceFixture, "Access token has shorter expiry than refresh token") {
...
TEST_CASE_FIXTURE(JwtServiceFixture, "Custom expiry configuration works") {
    ...
    int64_t access_duration = access_claims["exp"].asInt64() - access_claims["iat"].asInt64();
    int64_t refresh_duration = refresh_claims["exp"].asInt64() - refresh_claims["iat"].asInt64();
...
TEST_CASE_FIXTURE(JwtServiceFixture, "Token type claim distinguishes access and refresh") {
    auto access_token = service.generate_access_token(1, "user");
    auto refresh_token = service.generate_refresh_token(1);
...
TEST_CASE_FIXTURE(JwtServiceFixture, "Multiple tokens for same user can coexist") {
    auto token1 = service.generate_access_token(1, "user");
    auto token2 = service.generate_access_token(1, "user");
